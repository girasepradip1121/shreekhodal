...
import { 
  Home, Users, ShoppingCart, Package,Mail, Menu, LogOut,
  Tag
} from "lucide-react";
  ...
  return (
    <div>
      ...
      <div className={`fixed lg:relative top-0 left-0 h-screen bg-gray-800 text-white flex flex-col shadow-lg pt-16 w-72 transform 
        ${isOpen ? "translate-x-0" : "-translate-x-72"} transition-transform duration-300 lg:translate-x-0 lg:w-64 z-10`}>
        ...
        <nav className="flex flex-col p-4 space-y-1">
          {[  
            ...
            { to: "/admin/category", label: "Category", icon: <Tag size={20} /> },
            ...
            <NavLink 
              ...
              className={({ isActive }) => 
                `flex items-center space-x-2 p-3 rounded-lg transition ${isActive ? "bg-indigo-500 text-white" : "text-gray-300 hover:bg-indigo-600"}`
              }
            >
          ...
          <button 
            ...
            className="flex items-center space-x-2 p-3 rounded-lg hover:bg-red-700 text-white"
          >
            <LogOut size={20} />
            <span>Logout</span>
          </button>
...
const Navbar = ({ toggleSidebar }) => {
  return (
    <div className="w-full bg-gray-800 text-white flex items-center justify-between p-4 shadow-md fixed top-0 left-0 right-0 z-50">
      ...
      {/*Website Title */}
      <div className="text-xl font-bold">Worthy Vastra</div>
